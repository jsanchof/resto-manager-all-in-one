...
def create_reservation():
    if request.method == "POST":
        ...
        try:
            ...
            if data.get("table_id") and status_enum in [
        ...
        try:
            ...
            if status_filter:
                if status_filter in reservation_status.__members__:
                    stmt = stmt.where(
                        Reservation.status == reservation_status[status_filter]
                    )
                else:
                    return (
                        jsonify(
            ...
            if date_filter:
                try:
                    ...
                    stmt = stmt.where(
                        func.date(Reservation.start_date_time) == date_obj
                    )
                except ValueError:
                    return (
            ...
            stmt = (
                stmt.order_by(Reservation.start_date_time.desc())
...
def update_reservation(id):
    try:
        ...
        reserva.additional_details = data.get(
            "additional_details", reserva.additional_details
        )
