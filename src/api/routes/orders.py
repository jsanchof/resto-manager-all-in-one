...
def get_orders():
    try:
        ...
        return create_api_response(error="Invalid pagination parameters", status_code=400)
...
def create_order():
    try:
        ...
        return create_api_response(error=f"Missing required field: {str(e)}", status_code=400)
    except Exception as e:
        db.session.rollback()
